...
func TestRenderSimple(t *testing.T) {
	...
	r := NewRenderer(nil)
	out, err := r.Render(doc, NewContextFromAny(map[string]any{"name": "world"}))
	if err != nil {
	...
	_, err = r.Render(doc, NewContextFromAny(map[string]any{}))
	if err == nil {
...
func TestIfElifElse(t *testing.T) {
	...
	r := NewRenderer(nil)
	out, _ := r.Render(doc, NewContextFromAny(map[string]any{"a": true}))
	if out != "A" {
		t.Fatalf("a=true got %q", out)
	}
	out, _ = r.Render(doc, NewContextFromAny(map[string]any{"a": false, "b": true}))
	if out != "B" {
		t.Fatalf("b=true got %q", out)
	}
	out, _ = r.Render(doc, NewContextFromAny(map[string]any{"a": false, "b": false}))
	if out != "C" {
		t.Fatalf("else got %q", out)
	}
}
...
func TestForElse(t *testing.T) {
	...
	r := NewRenderer(nil)
	out, _ := r.Render(doc, NewContextFromAny(map[string]any{"items": []int{1, 2}}))
	if out != "-1-2" {
		t.Fatalf("got %q", out)
	}
	out, _ = r.Render(doc, NewContextFromAny(map[string]any{"items": []int{}}))
	if out != "empty" {
...
func TestSetAndUse(t *testing.T) {
	...
	r := NewRenderer(nil)
	out, err := r.Render(doc, NewContextFromAny(map[string]any{}))
	if err != nil {
		t.Fatalf("render error: %v", err)
	}
	if out != "hi" {
...
func TestInclude(t *testing.T) {
	...
	r := NewRenderer(ldr)
	out, err := r.Render(doc, NewContextFromAny(map[string]any{"x": 5}))
	if err != nil {
		t.Fatalf("render error: %v", err)
	}
	if out != "X[P5]Y" {
...
func TestExtendsAndBlocks(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("child parse error: %v", err)
	}
	r := NewRenderer(ldr)
	out, err := r.Render(childDoc, NewContextFromAny(map[string]any{"name": "Neo"}))
	if err != nil {
		t.Fatalf("render error: %v", err)
	}
