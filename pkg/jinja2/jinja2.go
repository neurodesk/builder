...
func (f *starlarkEmitter) emitStatement(s string) error {
	...
	if after, ok := strings.CutPrefix(s, "if "); ok {
		expr := after
		...
		return nil
	} else if after0, ok0 := strings.CutPrefix(s, "for "); ok0 {
		bind, lst, ok := strings.Cut(after0, " in ")
		if !ok {
		...
		return nil
	} else if after1, ok1 := strings.CutPrefix(s, "set "); ok1 {
		f.emit(after1)
...
func (j *Jinja2Evaluator) Eval(contents string, environment []starlark.Tuple) (string, error) {
	...
	for _, env := range environment {
		if len(env) != 2 {
			return "", fmt.Errorf("invalid environment tuple: %v", env)
		}
		key, ok := env[0].(starlark.String)
		if !ok {
			return "", fmt.Errorf("environment key must be a string: %v", env[0])
		}
		decl[key.GoString()] = env[1]
	}
